<template>
  <notifications
    group="notify"
    class="mono-notify !top-3 mt-safe-top !w-full px-3 max-md:!left-0 md:!w-[22rem] md:px-0 lg:!top-4 lg:!mt-0"
    :position="position"
  >
...
export default {
  ...
  computed: {
    ...
    i18n() {
      return {
        message: this.getTranslation(`${this.type}.default`),
      };
    },
  ...
  methods: {
    prepareMessage(code, type, text) {
      ...
      this.type = type;
      const localPath = `${type}.${code}`;

      if (text) {
        return text;
      }

      return this.getTranslation(localPath) !== localPath
        ? this.getTranslation(localPath)
        : this.i18n.message;
    },

    setPosition() {
      const classes = globalComponentConfig.UNotify?.positionClasses;
      const pageWidth = classes?.page ? document.querySelector(classes.page)?.offsetWidth : 0;
      const asideWidth = classes?.aside ? document.querySelector(classes.aside)?.offsetWidth : 0;
      const notifyWidth = document.querySelector(".mono-notify")?.offsetWidth;

      if (!pageWidth) return;

      const leftShift = asideWidth + pageWidth / 2 - notifyWidth / 2;

      document.querySelector(".mono-notify").style.setProperty("left", `${leftShift}px`);
    },
  },
};
</script>
