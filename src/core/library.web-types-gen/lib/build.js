...
async function extractInformation(absolutePath, config) {
  ...
  function getGlobalConfigJson(fileContents, componentName) {
    ...
    if (!cleanContents || hasUnclosedBraces(cleanContents)) return {};
...
function hasUnclosedBraces(inputString) {
  const stack = [];
  for (let i = 0; i < inputString.length; i++) {
    ...
    if (char === '{') {
      ...
      if (stack.length === 0 || stack.pop() !== '{') {
        return true; // Unmatched closing brace or no opening brace
      }
    }
  }
  return stack.length > 0; // Any remaining unclosed braces?
}
