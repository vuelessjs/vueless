...
const SlotTemplate = (args) => ({
  ...
  setup() {
    const icons = {
      Debit: require("vueless/ui.text-money/assets/debit.svg"),
      Credit: require("vueless/ui.text-money/assets/credit.svg"),
    };
  ...
  template: `
    <URow>
     <div class="${COMPONENT_CLASSES}" v-for="(weight, index) in weights" :key="index">
        <UMoney :weight="weight" v-bind="args">
          <template v-for="(slot) in slots" v-slot:[slot]>
            <template v-if="args[slot]">{{ args[slot] }}</template>
          </template>
        </UMoney>
  ...
  template: `
    <URow>
      <div class="${COMPONENT_CLASSES}"    v-for="(color, index) in colors" :key="index">
        <UMoney :color="color" v-bind="args">
          <template v-for="(slot) in slots" v-slot:[slot]>
            <template v-if="args[slot]">{{ args[slot] }}</template>
          </template>
        </UMoney>
...
const SymbolAlignTemplate = (args, { argTypes }) => ({
  ...
  setup() {
    return {
      args,
      symbolsAlign: argTypes.currencySymbolAlign.options,
    };
  },
  template: `
    <URow>
      <div class="${COMPONENT_CLASSES}" v-for="(symbolAlign, index) in symbolsAlign" :key="index">
        <UMoney
          :currencySymbolAlign="symbolAlign"
          v-bind="args"
        >
          <template v-for="(slot) in slots" v-slot:[slot]>
            <template v-if="args[slot]">{{ args[slot] }}</template>
